        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Paper.sphereCount >= 2)
            {
                ...
                else
                {
                    ...
                    Vector3 campos1 = Camera.allCameras[0].WorldToScreenPoint(pos1);
                    Vector3 campos2 = Camera.allCameras[0].WorldToScreenPoint(pos2);
                    float angle = Mathf.Acos(Vector2.Dot(campos2 - initialPosition, campos2 - campos1) / (((Vector2)(campos2 - initialPosition)).magnitude * (campos2 - campos1).magnitude));
                    float direction = Mathf.Sign(Vector3.Cross(campos2 - initialPosition, campos2 - campos1).z);
                    ...
                    if(cosine < 0)
                    {
                        Paper.usingPaper = Paper.paperList[Paper.paperList.Count - 2].GetComponent<Paper>();
                    }
                    else
                    {
                        Paper.usingPaper = Paper.paperList[Paper.paperList.Count - 1].GetComponent<Paper>();
                    }
                }
            }
        }
